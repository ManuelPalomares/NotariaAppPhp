...
class MenuApp {
    ...
    public function menuModulos($usuario) {
        ...
        foreach ($res as $opcion) {
            array_push($menus, array("text" => $opcion["DESCRIPCION"], "id" => $opcion["CODIGO"]));
        }
        return $menus;
    }
    ...
    public function menusSubmenus($usuario, $opcion) {
        ...
        $menus = array();
        foreach ($res as $opcion) {
            array_push($menus, array("text" => $opcion["DESCRIPCION"], "leaf" => true, "id" => $opcion["CODIGO"],'xtypealias'=>$opcion["WIDGETALIAS"]));
        }
        return $menus;
    }
    ...
    public function menuSubMenusJson($usuario, $opcion) {
        $result = null;
        $result["success"] = true;
        $result["modulos"] = $this->menusSubmenus($usuario, $opcion);
        echo json_encode($result);
    }
